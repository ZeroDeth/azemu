# azemu example: provider configuration
#
...
provider "azurerm" {
  ...
  # Point the provider at azemu's metadata endpoint. The provider fetches
  # cloud configuration from https://{metadata_host}/metadata/endpoints
  # and routes all subsequent ARM calls to the URLs azemu returns.
