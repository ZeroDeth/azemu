...
> **Single source of truth for vision, non-goals, resource roster, and
> milestones.** Concrete numbered tasks live in `TASKS.md`. Bugs and
...
1. **Fidelity over breadth.** Every resource azemu emulates has to
   ...
   unmodified `azurerm`. The `TODO.md` M1-M5 post-mortems are the receipts:
   five distinct classifier-class bugs we hit and fixed during the first
   end-to-end run, all documented so the next contributor gets the recipe
...
4. **Claude Code native, not Claude Code mandatory.** `.claude/agents`,
   `.claude/skills`, `.claude/rules`, `CLAUDE.md`, `AGENTS.md` are all
   first-class files. Contributors who use Claude Code get a productive
   environment in one command; contributors who do not are unaffected
   because the same rules are plain markdown and `AGENTS.md` follows the
   cross-vendor <https://agents.md> spec.

5. **Post-mortem discipline.** Every classifier-class bug (M1-M5 and
   whatever M6 turns out to be) earns a dated entry in `TODO.md` with
   symptom, root cause, fix, and regression test. No silent fixes. Over
   time, `TODO.md` becomes the regression corpus that keeps fidelity
   honest.

## Guiding principles

1. **Every fidelity claim is backed by a test.** No resource graduates
   from Stub to Full in `docs/PARITY.md` without a unit test pinning
...
3. **Contributors before maintainers.** `TASKS.md`, `TODO.md`, `docs/*`,
   `CLAUDE.md`, `AGENTS.md` are written so a brand-new contributor can
   ship their first resource on day one. If a workflow requires tribal
   knowledge that only the maintainer has, it is a documentation bug.
...
- **Not an `azcopy` target.** Storage data-plane work is delegated to
  ...
  [design note 1](docs/design-notes/0001-delegate-storage-data-plane-to-azurite.md).
...
- **Not a real Kubernetes control plane.** AKS is a management-plane
  ...
  documented in [design note 2](docs/design-notes/0002-azemu-plus-kind-for-aks-workload-deployments.md)
...
| `azurerm_storage_account` | `Microsoft.Storage/storageAccounts` | Full | ARM management plane, `listKeys`, `primaryEndpoints` rewrite. Data plane delegated to Azurite. See [design note 1](docs/design-notes/0001-delegate-storage-data-plane-to-azurite.md). |
...
| `azurerm_redis_cache` | `Microsoft.Cache/Redis` | Full (Standard tier) | Management plane in azemu, data plane delegated to `redis:7-alpine` sidecar. Premium-tier clustering and persistence are follow-ups. See [design note 3](docs/design-notes/0003-add-azure-cache-for-redis.md). |
...
- Phase 1 DONE: end-to-end `terraform apply` + `destroy` for RG + VNet + Subnet.
  Five classifier-class bugs fixed and post-mortemed (M1-M5 in `TODO.md`).
- Phase 2 DONE: per-package test coverage targets met. Integration suite
  walks token mint → OIDC discovery → JWKS signature verification end-to-end.
...
- New scenarios in `examples/terraform/scenarios/`:
  ...
  variant from [design note 2](docs/design-notes/0002-azemu-plus-kind-for-aks-workload-deployments.md)),
  `ado-pipeline/`.
...
| Azurite | Storage only (official Microsoft) | Data-plane fidelity for one service | azemu covers the management plane across many services and delegates the Storage data plane to Azurite as a docker-compose sidecar. See [design note 1](docs/design-notes/0001-delegate-storage-data-plane-to-azurite.md). |
...
```text
ROADMAP.md (this file)        vision, non-goals, resource roster, milestones
    |
    v
TASKS.md                      concrete numbered tasks per phase with status
    |
    v
TODO.md                       known gaps, unimplemented endpoints, bug post-mortems
    |
    v
docs/PARITY.md                per-resource Full/Stub/None matrix with test links

docs/design-notes/*.md        design notes (a notable choice and why; a new note supersedes an old one)
```

If a change breaks the ROADMAP, update this file first. Otherwise,
changes land in `TASKS.md` and `TODO.md`.
