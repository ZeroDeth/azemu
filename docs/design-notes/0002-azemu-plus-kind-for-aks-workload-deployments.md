...
Real-world deployment scenarios exercise more than ARM CRUD. A
representative production AKS workload (for example, a self-hosted
[expo-open-ota](https://github.com/expo/expo-open-ota) server running
three replicas with Redis-backed cache, Key Vault CSI for a bundle
signing key, and Blob storage for assets) needs:
...
3. **Unblocks deployment-shaped scenarios.** The Expo Updates server
   example above is one instance, but every "deploy real workload to
   AKS" scenario has the same shape: pods, CSI volumes, probes,
   ingress, network policy. Without a documented hybrid pattern, each
   scenario negotiates the same gap.
...
- AKS-workload scenarios have a documented, reproducible recipe instead
  of tribal knowledge.
- Deployment-style scenarios (CSI mounts, workload identity, network
...
2. **Defer all AKS workload scenarios indefinitely.** Rejected. Real
   contributor scenarios need this, and "we are an ARM emulator and
