...
event coordination. The OTA POC (ONA-56232) is the immediate trigger:
the expo-open-ota chart runs three replicas behind a load balancer with
`CACHE_MODE=redis`, backed by an Azure Cache for Redis instance.
...
mentions Cosmos DB and Event Grid. Contributors who want to round-trip a
Terraform config that includes Redis hit a wall.
...
- Outside Docker (flox workflow), Redis is an optional process; ARM
  ...
  that touch the Redis data plane pin the sidecar as a test dependency.
...
2. **OTA-class deployment scenarios need it.** ONA-56232 is one
   ticket; any multi-replica AKS workload with shared state lands in
   the same shape. Without Redis, the `aks-workload/` scenario from ADR
   0002 cannot fully express what production deployments do.
...
- The OTA-class scenario in ADR 0002 becomes fully expressible without
...
2. **Skip Redis entirely; tell users to wire their own outside
   `docker-compose.yml`.** Rejected. The OTA-class scenario in ADR
...
4. **Defer Redis to v0.4.** Rejected. The OTA POC scenario is in flight
   now; deferring leaves the "azemu cannot exercise this real ticket"
   gap open for a full milestone.
...
- `examples/terraform/scenarios/aks-workload/`: integrates the Redis
  cache into the OTA-class scenario from ADR 0002.
