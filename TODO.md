...
| M8 | `three-tier` apply fails: `azurerm_lb_probe.http` -> `Provider produced inconsistent result after apply: ... Root object was present, but now absent`. | `Microsoft.Network/loadBalancers/probes` (and `/loadBalancingRules`) have NO standalone create/update REST operation (confirmed on Microsoft Learn: they are nested sub-resources of the parent Load Balancer). The azurerm provider writes a probe by PUTting the PARENT LB with the probe inline in `properties.probes`, then reads it back from the LB. But `putLB` in `internal/arm/lb.go` DELETES the inline `probes`/`loadBalancingRules` arrays (it expected them via the standalone `.../probes/{name}` PUT handler, which real azurerm never calls), so `getLB` returns none and the provider sees the probe vanish. Same false-positive class as M4: the standalone probe/rule handlers were exercised by hand-built requests, never by real azurerm. | OPEN | Persist inline `probes`/`loadBalancingRules` from the parent LB PUT (either keep them inline in LB properties and return them from `getLB`, or explode them into child store entries the existing `getLB` re-assembly already reads) and sync removals on update. Needs a plan (touches `internal/arm/lb.go` putLB/getLB + `lb_test.go`) and terraform-capable E2E verification. Backend address pools are unaffected: `azurerm_lb_backend_address_pool` IS a real standalone ARM sub-resource. |
