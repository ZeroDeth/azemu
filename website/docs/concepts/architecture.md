...
azemu multiplexes them on the ARM port (`:4566`) behind a host check, the
same way real Azure serves them from distinct hostnames:
...
- `{endpoint}.azureedge.net` serves the classic CDN content data plane. The
  handler resolves the CDN endpoint from the host, finds its Blob origin
  ...
  metadata by default.
- `{endpoint}.azurefd.net` serves the Front Door content data plane. The
  handler resolves the afdEndpoint from the host, walks the Front Door
  resource graph (afdEndpoint -> route -> originGroup -> origin) to find the
  Blob origin, and reverse-proxies the request to Azurite the same way as
  classic CDN, reusing the same blob proxy (`GET`/`HEAD` only).
